...
struct GPUPool {
    ...
    ~GPUPool() {
        ...
        for (uint32_t i = 0; i < MAX_TOWERS; ++i) {
            ...
            if (d_twiddles[i]) cudaFree(d_twiddles[i]);
    ...
    void init() {
        ...
        for (uint32_t i = 0; i < MAX_TOWERS; ++i) {
            ...
            cudaMalloc(&d_twiddles[i], MAX_RING * sizeof(uint64_t));
