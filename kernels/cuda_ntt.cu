...
__global__ void scale_by_ninv(uint64_t* __restrict__ x, uint64_t n_inv,
                               uint64_t q, uint32_t N) {
    uint32_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void ntt_stage_dit(uint64_t* __restrict__ x, const uint64_t* __restrict__ wpow,
    ...
    uint64_t w     = wpow[N_half / half_m * j];
    uint64_t u     = x[i_top];
